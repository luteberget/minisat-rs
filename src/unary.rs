...
impl Unary {
    ...
    /// Return a `Bool` which represents whether the number is less than a given non-negative integer constant.
    pub fn lt_const(&self, x :isize) -> Bool {
        !(self.gte_const(x))
    }

    /// Return a `Bool` which represents whether the number is less than or equal to a given non-negative integer constant.
    pub fn lte_const(&self, x :isize) -> Bool {
        self.lt_const(x+1)
    }

    /// Return a `Bool` which represents whether the number is greater than or equal to a given non-negative integer constant.
